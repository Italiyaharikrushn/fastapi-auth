...
@router.post("/", response_model=OrderResponse)
def create_order(order_data: OrderCreate, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    return crud_order.create_order(db, user.id, order_data)
...
def get_all_orders(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
